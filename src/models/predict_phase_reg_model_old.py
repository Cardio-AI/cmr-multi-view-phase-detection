...
def predict(cfg_file, json_file=None, data_root='', c2l=False, exp=None, number_of_examples=None):
    ...
    if c2l:
        # SKM combine: Add flag/information if it is lax or sax to decide here
        config['DATA_PATH_CMR'] = os.path.join(data_root, 'sax')
    ...
    x_train_sax, y_train_sax, x_val_sax, y_val_sax = get_trainings_files(data_path=config['DATA_PATH_CMR'],
                                                                         suffix=suffix,
                                                                         ftype=file_ending,
                                                                         path_to_folds_df=config['DF_FOLDS'],
                                                                         fold=config['FOLD'])

    # SKM combine: Add here in string if LAX or SAX and also in naming convention below
    logging.info('SAX train CMR: {}, SAX train masks: {}'.format(len(x_train_sax), len(y_train_sax)))
    logging.info('SAX val CMR: {}, SAX val masks: {}'.format(len(x_val_sax), len(y_val_sax)))

    chunk_size = 10
    x_train_saxs = [x_train_sax[i:i + chunk_size] for i in range(0, len(x_train_sax), chunk_size)]
    y_train_saxs = [y_train_sax[i:i + chunk_size] for i in range(0, len(y_train_sax), chunk_size)]
    x_val_saxs = [x_val_sax[i:i + chunk_size] for i in range(0, len(x_val_sax), chunk_size)]
    y_val_saxs = [y_val_sax[i:i + chunk_size] for i in range(0, len(y_val_sax), chunk_size)]

    logging.info('Split into chunks of:')
    logging.info('SAX train CMR: {}, SAX train masks: {}'.format(len(x_train_saxs), len(y_train_saxs)))
    logging.info('SAX val CMR: {}, SAX val masks: {}'.format(len(x_val_saxs), len(y_val_saxs)))
    ...
    if use_segmentation is None or use_segmentation is False:
        NNUNET_SEG = False
        val_config['PRECOMPUTED_MASKS'] = False

    else:
        NNUNET_SEG = dataset_json.get("seg_model_path", False)
        val_config['PRECOMPUTED_MASKS'] = NNUNET_SEG
    ...
    junk = 0
    # SKM combine: change naming conventions here
    for x_train_sax_, y_train_sax_, x_val_sax_, y_val_sax_ in zip(x_train_saxs, y_train_saxs, x_val_saxs, y_val_saxs):
        preds_, moved_, vects_, gts_, segmentations_ = [], [], [], [], []
        logging.info('***********  processing junk: {} of {}'.format(junk, len(x_val_saxs)))
        validation_generator = PhaseRegressionGenerator_v2(x_val_sax_, y_val_sax_, config=val_config, in_memory=False, dataset_json=dataset_json)
...
def write_4d_files_to_disk(examples, focus_size, PRETRAINED_SEG, config, example_path, moved, segmentation,
                           vects, x_val_sax, norm_thresh=55, connected_component_filter=None, mask_channels=None):
    import SimpleITK as sitk
    import os
    from src.models.predict_phase_reg_model_old import interpret_deformable
    ...
    for example in examples:
        ...
        #SKM combine: Add here the option between SAX and LAX
        zeros[:, :,
        int(ct[0] - focus_size):int(ct[0] + focus_size),
        int(ct[1] - focus_size):int(ct[1] + focus_size)] = 1
        # SAX code
        # zeros[:, int(ct[0] - focus_size):int(ct[0] + focus_size),
        ...
        # SKM Combine: add here option for LAX/SAX
        spacing = config.get('SPACING', (2.5, 2.5))
        spacing = list(reversed(spacing)) + [1.0, 1.0]
        # SAX code:
        # spacing = config.get('SPACING', (2.5, 2.5, 2.5))
        # spacing = list(reversed(spacing)) + [1.0]

        elem = x_val_sax[example]
        ...
        if PRETRAINED_SEG:
            from src.models.predict_phase_reg_model_old import seg_based_direction
            ...
            seg_based_direction(vects[example], moved[example], segmentation[example], x_val_sax[example],
...
def seg_based_direction(vect, moved, segmentation, x_val_sax, focus_size, example_path, config,  targetfile_type='nii'):
    import SimpleITK as sitk
    import os
...
def get_rv_outline_as_mask(masks, include_septum=False):
    ...
    for timestep in range(rv_mask.shape[0]):
        # SAX Code with z-slices:
        # for z in range(rv.shape[1]):
        #     rv_slice = rv[ts, z]
        ...
        #     new_slice = np.logical_and(new_slice, rv[ts, z])
        #     faked_rv_myo[ts, z] = new_slice
...
def get_focus_point(mask2d: np.ndarray, calculation: Union[Literal['septum'], List, int, None] = 'septum',
                    z: int = None, print_ct=False, whole_mask=True):
    from scipy import ndimage
    # SKM Combine: Where is the option for septum?
    if type(calculation) == int:
        calculation = [calculation]
    # SKM Combine: mask2d without [..., None] in SAX
...
def interpret_deformable(vects_nda, masks=None, mask_channels=None, dir_axis=0, length=None, filename=None,
                         norm_percentile=55, z=None, diff_thresh=1.2,  component_padding=None, sigma=0.8, as_angle=False,
    ...
    ax=(1, 2)
    directions_ma, dir_1d_mean = get_masked_array(directions, mask, axis=ax)
    norm_ma, norm_1d_mean = get_masked_array(norm_nda, mask, axis=ax)
...
def get_directions(ct, dim_, length, vects_nda, diff_thresh=1.2, masked=False, dir_axis=0, as_angle=False,
    ...
    if as_angle:
        ...
        directions = flow2direction_lambda([vects_nda[..., dir_axis:], centers_tensor[..., dir_axis:]])[..., 0].numpy()  # remove the last extra channel
    ...
    dir_rest = scipy.ndimage.gaussian_filter(dir_rest, sigma=sigma, mode='wrap')  # no border wrapping for the rest sequence
...
def predict_phase_from_deformable(exp_path, create_figures=True, norm_thresh=50, dir_axis=0, roll_by_gt=True,
    ...
    from src.visualization.Visualize import plot_dir_norm, plot_dir_norm_split_by, plot_pfd_per_phase_as_violin, plot_scatter
    ...
    # SKM Combine, change here to reference to given mask_channels in dataset_json
    ...
    # nda_vect, gt_len, gt, dir_axis, norm_thresh, idx, filename=None, ct_calculation='septum', masks=None, mask_channels=None
...
def interpret_deformable_async(nda_vect, gt_len, gt, dir_axis, norm_thresh, connected_component, idx, filename=None,
    ...
    from src.utils.detect_phases_from_dir import detect_phases
    indices = detect_phases(dir_1d_mean=dir_1d_mean[:cardiac_cycle_length], gt_phases=ind)
